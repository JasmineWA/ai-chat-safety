...
DEFAULT_TRAIN_PATH = os.path.join(SCRIPT_DIR, "datafiles", "output_pku_saferlhf_train_zh.json")
DEFAULT_EVAL_PATH = os.path.join(SCRIPT_DIR, "datafiles", "output_pku_saferlhf_eval_zh.json")
DEFAULT_OUTPUT = os.path.join(SCRIPT_DIR, "models", "risk_classifier_output_pku_zh")
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA fine-tuning for the Chinese PKU output safety classifier")
...
def print_label_stats(title, label_list, counts, label2id):
    ...
    for label in label_list:
        ...
        bar = "#" * max(1, count // 500) if count else ""
...
def build_misclassified_samples(eval_items, label_list, y_true, y_pred, predictions):
    results = []
    for idx, (true_id, pred_id) in enumerate(zip(y_true, y_pred)):
        if int(true_id) == int(pred_id):
            continue

        probs = torch.softmax(torch.tensor(predictions[idx]), dim=-1).tolist()
        item = eval_items[idx]
        results.append(
            {
                "index": idx,
                "expected_label": label_list[int(true_id)],
                "predicted_label": label_list[int(pred_id)],
                ...
                "is_refusal_safe": bool(item.get("is_refusal_safe", False)),
                "probabilities": {
                    label_list[i]: round(float(prob), 6)
...
def build_misclassification_summary(misclassified_samples):
    direction_counter = Counter()
    category_counter = Counter()
    source_counter = Counter()

    for item in misclassified_samples:
        direction = f"{item['expected_label']} -> {item['predicted_label']}"
        direction_counter[direction] += 1
        category_counter[item.get("category", "")] += 1
        source_counter[item.get("dataset_source", "")] += 1

    return {
        "total_misclassified": len(misclassified_samples),
        "by_direction": dict(direction_counter),
        "by_category": dict(category_counter),
        "by_dataset_source": dict(source_counter),
    }
...
def main():
    ...
    print("=" * 60)
    print("LoRA fine-tuning for Chinese PKU output safety classifier")
    ...
    eval_items = load_json(args.eval_path)
    ...
    misclassified_summary = build_misclassification_summary(misclassified_samples)
    ...
    with open(
        os.path.join(args.output_dir, "misclassified_eval_samples.json"),
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(misclassified_samples, f, ensure_ascii=False, indent=2)

    with open(
        os.path.join(args.output_dir, "misclassified_eval_summary.json"),
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(misclassified_summary, f, ensure_ascii=False, indent=2)

    print("\nmisclassified summary:")
    print(json.dumps(misclassified_summary, ensure_ascii=False, indent=2))
